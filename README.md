...
**支持平台（22 个）：智学网、七天网络、好分数、五岳阅卷、华翰云、光大阅卷、云阅卷、新教育、润建学情、54学霸、九科星、慧阅卷、乐华阅卷、鑫考、慧学星、粤教翔云、云阅卷(好分数)、科耘阅卷、光大阅卷V2、威科姆（悦卷通）、C30教育云、AMEQP网上评卷**  如果您有适配需求，请联系我。
...
### 工作流配置
- 内置快速批改 / 普通批改 / 双评模式三种工作流
- 支持自定义工作流：自由组合供应商、模型、思考链深度
...
### 评分理由编辑
- AI 评分后可手动编辑评分理由，支持 Markdown 格式和实时预览

### 批阅份数控制
- 可设置目标批阅份数，达到后自动暂停，方便控制工作量

### 配置备份与恢复
- 所有配置支持导出为 JSON 文件，更换设备一键恢复
...
### v1.21.11 (2026-09-08)
**新平台**
- 新增 C30教育云（大数据精准教学系统）平台适配，支持 Canvas 答题卡渲染和 Vue + Element UI 组件交互

**优化**
- 页面加载时自动检测满分值并写入配置，无需手动设置
- 暴露 PresetManager 到全局，支持适配器在 onPageLoad 中读写配置

### v1.21.10 (2026-08-31)
**新平台**
- 新增威科姆（悦卷通）平台适配，支持多小题评分、OSS 图片和键盘/鼠标/步骤三种打分模式

**新功能**
- 新增评分理由编辑功能及预览区，支持 Markdown 格式

**修复**
- 修复七天网络新域名（qt7.cn）无法弹出 AI 阅卷窗口的问题

### v1.21.9 (2026-06-18)
**新平台**
- 新增科耘阅卷平台适配，支持 SVG 图片渲染和 Element UI 组件
- 新增慧学星平台适配，支持 OSS 图片托管和分步骤评分模式
- 新增粤教翔云智慧测评平台适配，支持题组批改模式
- 新增云阅卷(好分数)平台适配，Vue 3 + Element Plus 框架
- 新增光大阅卷 V2 平台适配，支持 IP:端口部署版本

**优化**
- 科耘适配器初始化时自动取消"打分键盘"勾选，避免键盘输入被捕获导致误填分数
- 科耘适配器采用延迟填分模式，用户确认后才填入分数触发自动提交
- 科耘适配器提交前自动勾选"自动提交"，确保填分后平台能正常提交

**修复**
- 修复五岳阅卷平台更新后无法获取答题卡图片的问题，改用 DOM 结构匹配替代 URL 模式匹配

**文档**
- 更新文档站，平台数量从 14 个扩展到 19 个

### v1.21.8 (2026-05-29)
**新平台**
- 新增54学霸阅卷平台适配，支持 Canvas/base64 图片渲染和多小题输入框评分
- 新增九科星阅卷平台适配，支持多小题评分和图片裁剪功能
- 新增慧阅卷平台适配，支持 frame 内部文档访问和评分功能
- 新增乐华阅卷平台适配

**新功能**
- 配置备份与恢复功能，确保更新时配置安全
- 评阅历史面板支持 Markdown 渲染，优化显示效果
- 评分弹窗评语支持滚动显示长评语
- 纠错面板添加 Markdown 编辑器预览功能，支持 AI 润色
- 双评阈值支持设为0，优化仲裁提示词

**优化**
- AI 评分时自动忽略涂改、划掉的内容，只评判最终保留的答案
- 重构 Markdown 渲染器，支持 LaTeX 块级和行内公式
- 优化图片获取逻辑，增加有效性验证和超时处理
- 改进分数匹配逻辑，优先精确匹配并增加模糊匹配策略
- 增强分数解析逻辑，支持负数和超出满分的处理
- 优化多个编辑面板的交互体验
- 阻止 Enter 键冒泡，避免触发表单提交
- 优化各面板 z-index 属性，确保覆盖层优先级正确

**修复**
- 修复五岳阅卷答题卡获取问题和多图片显示问题
- 修复光大适配器多个稳定性问题
- 修复乐华阅卷等待获取下一份试卷失败的问题
- 修复乐华阅卷分数填充逻辑
- 修复 OOBE 界面编辑题目后无法显示的问题

### v1.21.7 (2026-05-20)
**新平台**
- 新增润建学情大数据精准分析平台适配，支持 CSS 背景图渲染和点击式评分
...
### v1.21.4 (2026-05-15)
**新平台**
- 新增云阅卷平台适配，支持内网阅卷系统的自动批改
- 新增新教育智能平台适配，支持考试系统和作业系统两种批改模式

**新功能**
- 批改进度面板支持拖动，可自由调整位置并自动保存，避免遮挡网页原有内容
- 新手引导验证密钥时显示加载动画
- 新手引导验证密钥时自动清除之前的错误提示

**优化**
- API 密钥验证速度优化，使用固定回复减少响应时间
- 批改进度面板圆角样式优化
